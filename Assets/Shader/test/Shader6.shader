Shader "Unlit/Shader1"
{
        ...
        SubShader
    {
        Tags { "RenderType" = "Transparent" // tag to inform unity pipeline what type this is
               "Queue"= "Transparent" // render order; skybox - opaque - transparent - overlay
        ...
        { //hlsl coding language 
            Cull Off // Cull means get not render something, eg Cull front means dont render the front side, the default of Cull is Cull Back(dont render the back), Cull off
            // means turn off Cull, render both sider of the mesh.
            ZWrite Off // change the depth buffer
            Blend One One //addtive, define the num in blend formular in order to make create addtive blending
            // formular Src*NUM1 +/- Dst*NUM2, if NUM12 both equals to 1, then it is the additive blending, if NUM1 = Dst, NUM2=0, (default +), then it's multipy blending
            //Blend DstColor Zero //Multipicated
            CGPROGRAM
            ...
            {   // float4 myValue;
               //  float2 othervalue = myValue.gr; //swizzling
                ...
                // Inverselerp
                //saturate means to clamp the value, the value under 0 would be clamped at 0, the value upper 1 will be clamped at 1
                //float4 t = saturate(Inverselerp(_ColorStart, _ColorEnd, i.uv0.x));
                //frac is a functionh to make stuff repeat between 0 and 1
                //t = frac(t);
                //return t;
            ...
            // color = vertex, use colors to see how the uv is mapped on the mesh 
                //_time is a global variable in unty, xyzw component use different scale of time,  y is seconds ,w is seconds divded by 20
                ...
                float yOffset = cos(i.uv0.x * TAU * 5) * 0.01;

                float t = cos((i.uv0.y + yOffset -_Time.y * 0.1) * 5 * TAU)*0.5 + 0.5;//TAU make sure that the cos is in a whole peried
                ...
                float4 colorlerpstation = lerp(_ColorStart, _ColorEnd, i.uv0.y);
                return gradient*waves*colorlerpstation;
